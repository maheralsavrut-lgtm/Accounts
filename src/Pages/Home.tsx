...
export default function Home({ mode }: { mode: 'login' | 'signup' }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        navigate(`/profile`); // سيتم استبداله بـ ID لاحقاً
      } else {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-6 font-sans italic">
      <motion.div 
        layout
        ...
        <div className="space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="relative">
              <input type="email" placeholder="البريد الإلكتروني" required className="w-full bg-white/[0.02] border border-white/10 p-4 pr-11 rounded-2xl focus:border-royal-blue/40 outline-none text-sm text-white transition-all" value={email} onChange={(e) => setEmail(e.target.value)} />
            ...
            {!isLogin && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-3">
                <div className="relative">
                  <input type="email" placeholder="تأكيد البريد الإلكتروني" required className={`w-full bg-white/[0.02] border ${!isEmailMatch ? 'border-red-500/50' : 'border-white/10'} p-4 pr-11 rounded-2xl outline-none text-sm text-white`} value={confirmEmail} onChange={(e) => setConfirmEmail(e.target.value)} />
                  <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-600" size={16} />
                </div>
                <div className="relative">
                  <input type="tel" placeholder="رقم التليفون (اختياري)" className="w-full bg-white/[0.02] border border-white/10 p-4 pr-11 rounded-2xl focus:border-royal-blue/40 outline-none text-sm text-white" value={phone} onChange={(e) => setPhone(e.target.value)} />
            ...
            {!isLogin && (
              <div className="space-y-4 pt-1">
                ...
                {/* Checkboxes بالشكل الاحترافي */}
                <div className="space-y-3 px-1">
                  {[
                    { state: agreedTerms, set: setAgreedTerms, text: "أوافق على شروط الاستخدام", link: "/legal" },
